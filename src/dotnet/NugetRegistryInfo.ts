...
but the environment variable is empty or undefined.`)
   ...
   */
  private static isNextVersionAlreadyPublished(source: string, packageId: string, nextVersion: string): Promise<boolean> {
    const nodeCommand = this._getIsNextVersionAlreadyPublishedCommand(source, packageId, nextVersion)
   ...
   * @param source The NuGet Source to check for an existing package with next
  ...
  private static _getIsNextVersionAlreadyPublishedCommand(source: string, packageId: string, nextVersion: string): string {
