...
export class NugetRegistryInfo {
  ...
  private get canPushPackagesToUrl(): Promise<true> {
    ...
    this._canPushPackagesToUrl = finalResult;
...
but the environment variable is empty or undefined.`);
  ...
  GetPackCommand(
    ...
    if (usePerSourceSubfolder)
      validOpts.output = join(
        validOpts.output,
        NugetRegistryInfo.GetNameForURL(this.url),
        sep,
      );
    if (usePerPackageIdSubfolder)
      validOpts.output = join(
        validOpts.output,
        this._project.Properties.PackageId,
        sep,
      );
