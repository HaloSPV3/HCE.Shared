...
async function commonChecks(options: Options) {
    ...
    await test("options is defined", () => void notStrictEqual(options, undefined));
    await test("options.preset is conventionalcommits", () => void strictEqual(options.preset, "conventionalcommits"));
...
await describe('HCE.Shared module returns semantic-release shareable configuration (Options object)',
    async () => void await commonChecks(HceShared())
)
