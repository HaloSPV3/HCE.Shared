  ...
  await it('given a class, can filter a PropertyDescriptors object to an array of getters\' property descriptors', () => {
    deepStrictEqual(
      filterForGetters(
        ...
        Object.getOwnPropertyDescriptors(class {
          static #var0: string
...
await describe('getOwnPropertyDescriptors', async (c00) => {
  ...
  await it('can be used to list all own instance getters of a class', () => {
    let propNames = getOwnPropertyDescriptors(
      NugetProjectProperties, true, false,
    ).map(
      o => Object.entries(o),
    ).flat().filter(
      e => typeof e[1].get === 'function' && e[0] !== '__proto__',
    ...
    propNames = getOwnPropertyDescriptors(
      MSBuildProjectProperties, true, false,
    ).map(
      o => Object.entries(o),
    ).flat().filter(
      e => typeof e[1].get === 'function' && e[0] !== '__proto__',
  ...
  await it('can be used to list all own static getters of a class', () => {
    const propNames = getOwnPropertyDescriptors(
      NugetProjectProperties, false, false,
    ).map(
      o => Object.entries(o),
    ).flat().filter(
      e => typeof e[1].value === 'function',
...
await describe('isConstructor', async (c00) => {
  ...
  await it('returns true for a ConstructorLike arg', () => {
    ...
    deepStrictEqual(
      isConstructor(MSBuildProjectProperties),
      true,
    )
    deepStrictEqual(
