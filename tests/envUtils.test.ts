...
await describe('envUtils', async () => {
  await describe('getEnvVarValue', async () => {
    ...
    await it('can load .env file and variable value in process.env is identical to value in .env file', { todo: true })
  })

  await describe('getEnv', async () => {
    await it('returns ProcessEnv object', () => {
      function isProcessEnv(o: object): o is NodeJS.ProcessEnv {
        return Object.keys(o).every(v => typeof v === 'string') && Object.values(o).every(v => v === undefined || typeof v === 'string')
      }
